...
import { createUserCar, fetchCarByCompany } from '../../../../store/Actions/carAction';

const CarContainer = ({imageLink,name,id,handleCarAdd}) => (
    
  <Pressable onPress={()=>handleCarAdd(id)} style={chooseStyle.carContainer}>
    <Image
...
const ChooseCar = ({currentComponent,setCurrentComponent,dispatch,companyId,companyName,setCompanyId,setCompanyName}) => {
  ...
  useEffect(()=>{
    dispatch(fetchCarByCompany(companyId));
  },[companyId])
...
const handleCarAdd=(id)=>{
  dispatch(createUserCar(id));
  setCurrentComponent('select-car');
}
  return (
    <>
...
<Pressable onPress={()=>{
  setCompanyId(null);
  setCompanyName('');
  setCurrentComponent('choose-company')
}} style={chooseStyle.backButton}>
...
<Text style={chooseStyle.companyText2}>Cars by {companyName}</Text>
    </View>
      ...
      <View style={chooseStyle.companySelectContainer}>
        <BottomSheetFlatList
          data={data}
          renderItem={({item}) => (
            <CarContainer imageLink={item?.imageLink} name={item?.model}
            id={item?.id}
